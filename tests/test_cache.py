...
def test_get_sound_by_id_success(mock_get) -> None:  # type: ignore
    mock_response = {
        'id': 123,
        'name': 'test sound',
...
@patch('utils.cache.requests.get')
def test_get_sound_by_id_failure(mock_get) -> None:  # type: ignore
...
def test_load_cache_valid(mock_get_sound, mock_file, mock_exists) -> None:  # type: ignore
...
@patch('utils.cache.os.path.exists')
def test_load_cache_no_file(mock_exists) -> None:  # type: ignore
...
def test_save_cache(mock_dump) -> None:  # type: ignore
    cache = cast(Dict[str, CacheEntry], {'filename': {'good_ids': ['1', '2'], 'last_updated': '2023-01-01'}})
    save_cache(cache)
